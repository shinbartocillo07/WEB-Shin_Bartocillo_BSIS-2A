...
$trip_stats = $conn->query("
    SELECT
        ...
        COUNT(CASE WHEN status IN ('dropped_off', 'completed') THEN 1 END) as completed_trips,
...
$monthly_trips = $conn->query("
    SELECT
        ...
        COUNT(*) as trip_count,
        COUNT(CASE WHEN status IN ('dropped_off', 'completed') THEN 1 END) as completed_count
    FROM trips
...
<body>
    <div class="dashboard">
        ...
        <h2>Today's Trip Completion</h2>
        <?php
        $today_completion = $conn->query("
            SELECT
                COUNT(*) as total_today,
                COUNT(CASE WHEN status IN ('dropped_off', 'completed') THEN 1 END) as completed_today,
                COUNT(CASE WHEN status IN ('picked_up', 'in_transit') THEN 1 END) as in_progress_today
            FROM trips
        ...
        <div class="completion-summary">
            <div class="completion-card">
                ...
                <div class="progress-bar">
                    <div class="progress-fill" style="width: <?php echo $today_completion['total_today'] > 0 ? ($today_completion['completed_today'] / $today_completion['total_today']) * 100 : 0; ?>%"></div>
                </div>
                <p><?php echo $today_completion['completed_today']; ?> of <?php echo $today_completion['total_today']; ?> trips completed</p>
                <p><?php echo $today_completion['in_progress_today']; ?> trips in progress</p>
            </div>
        </div>
            ...
            <tbody>
                <?php while ($trip = $recent_trips->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><span class="status-<?php echo $trip['status']; ?>"><?php 
                            $status_text = ucfirst(str_replace('_', ' ', $trip['status']));
                        ...
                        <td><?php echo ($trip['dropoff_time'] && in_array($trip['status'], ['dropped_off', 'completed'])) ? date('H:i', strtotime($trip['dropoff_time'])) : '-'; ?></td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
